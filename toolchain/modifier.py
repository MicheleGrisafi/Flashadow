...
unsafeCall  = re.compile('CALL\s+({}|{}|{}|{}|{}|{})\s+'.format(regMode,indexMode,indirectMode,symbolicMode,absoluteMode,immediateMode),re.I)
unsafeCalla = re.compile('CALLA\s+({}|{}|{}|{}|{}|{})\s+'.format(regMode,indexMode,indirectMode,symbolicMode,absoluteMode,immediateMode),re.I)
...
while line:    
    ...
    if(match):
        ...
        returnInstructions+=1
    elif(match := unsafeReta.search(line)): #TODO: instrument reta and use that
        newEntry = execReplace(match,safe_ret,mode=NONE,offset=0)
        returnInstructions+=1
