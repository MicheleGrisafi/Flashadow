...
counter = 0
# NO NEED TO INSERT NOP AFTER CALL! WE USE SHADOW STACK

def insertNopFunction(match):
    logging.debug("Found function defition: {} ".format(match.group(0)))
...
for f in sys.argv[2:]:
    inputName = f
    input = open(inputName, 'r')
    logging.debug("Parsing file {} adding NOP slides after function declarations".format(inputName))
    modifiedMain = open(inputName+".tmp", 'w')

    content = input.read()

    content = re.sub(functionInstruction,insertNopFunction,content)

    modifiedMain.write(content)
